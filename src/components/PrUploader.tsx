...
export default function PrUploader({ onDataLoaded, currentCount, lang }: PrUploaderProps) {
  ...
  const buildExtraRawFields = (sheet: XLSX.WorkSheet, rowCount: number): Record<string, any>[] => {
    ...
    const idxTotalKgs = findIdx("Total Kgs");
    const idxCbmDirect = findIdx("Total CBM");
    ...
    for (let i = 0; i < rowCount; i++) {
      out.push({
        ...
        totalKgsRaw: getNum(i, idxTotalKgs),
        cbmDirectRaw: getNum(i, idxCbmDirect),
  ...
  const applyMapping = (rows: any[], currentMap: Record<string, string>, extraFieldsOverride?: Record<string, any>[]) => {
    try {
      ...
      const parsedEntries: PrEntry[] = rows.map((row, idx) => {
        ...
        // CBM/weight basis: the source file now provides two separate
        // columns instead of the old single "Total CBM/KG" column —
        // "Total Kgs" and "CBM". Which one applies depends on the item
        // type, inferred from the Item code's first letter:
        //   "A..." = Accessories -> use the "Total Kgs" column
        //   "C..." = Fabric      -> use the "CBM" column
        // Both are treated as already-total (per-line) figures, not
        // per-unit, matching how the old combined column worked. Falls
        // back to Unit Weight × Qty, then a flat 0.003 CBM/unit constant,
        // for any item code that isn't A/C-prefixed or when the relevant
        // column is missing/blank for that row.
        let cbm: number;
        const itemTypePrefix = itemCode.trim().charAt(0).toUpperCase();
        if (itemTypePrefix === "A" && extra.totalKgsRaw !== undefined && extra.totalKgsRaw > 0) {
          cbm = extra.totalKgsRaw;
        } else if (itemTypePrefix === "C" && extra.cbmDirectRaw !== undefined && extra.cbmDirectRaw > 0) {
          cbm = extra.cbmDirectRaw;
        } else if (extra.unitWeightRaw !== undefined && extra.unitWeightRaw > 0 && qty > 0) {
          cbm = extra.unitWeightRaw * qty;
        } else {
          cbm = qty * 0.003; // Last-resort fallback only, when nothing else is available
        }
